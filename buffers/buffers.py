...
class MeanFieldRolloutBufferSamples(NamedTuple):
    observations: torch.Tensor
    nearby_observations: torch.Tensor
    actions: torch.Tensor
    old_i_values: torch.Tensor
    old_n_values: torch.Tensor
    old_g_values: torch.Tensor
...
class RolloutBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        self.n_rewards, self.n_advantages, self.g_rewards, self.g_advantages = None, None, None, None
...
class MeanFieldRolloutBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def reset(self) -> None:
        ...
        self.nearby_observations = np.zeros((self.buffer_size, self.n_envs, self.state_dim), dtype=np.float32)
        self.observations = np.zeros((self.buffer_size, self.n_envs, self.state_dim), dtype=np.float32)
        self.actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(
        ...
        # Reshape needed when using multiple envs with discrete observations
        # as numpy cannot broadcast (n_discrete,) to (n_discrete, 1)
        self.nearby_observations[self.pos] = np.array(nearby_obs).copy()
        self.observations[self.pos] = np.array(obs).copy()
        self.actions[self.pos] = np.array(action).copy()
        self.rewards[self.pos] = np.array(reward).copy()
        self.n_rewards[self.pos] = np.array(n_reward).copy()
