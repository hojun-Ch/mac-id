...
from utils import state_engineering,  obs_to_single_reward, make_observation_mf, make_env, check_success_rate, shortest_distance
...
if __name__ == '__main__':
    ...
    for j in range(args.max_step // args.rollout_length):
        ...
        raw_obs = env.reset()
        obs, n_obs= make_observation_mf(raw_obs[0], args.map_length, args.map_width, args.num_ped, args.obs_dim, args.dummy_index, args.neighbor_distance)
        ...
        for i in range(args.rollout_length):
            ...
            new_prev_n_obs = state_engineering(prev_n_obs, args.map_length, args.map_width, args.num_ped, args.obs_dim)
            action, log_prob, value = agent.act(torch.from_numpy(new_prev_obs),torch.from_numpy(new_prev_n_obs))
            ...
            if i == 0:
                agent.rollout_buffer.add(new_prev_obs, new_prev_n_obs,action, reward, reward, g_reward, np.array([1] * args.num_ped), value, value, value, log_prob, np.array([0.0] * args.num_ped))
            else:
                agent.rollout_buffer.add(new_prev_obs, new_prev_n_obs,action, reward, reward, g_reward, np.array([0] * args.num_ped), value, value, value, log_prob, np.array([0.0] * args.num_ped))
        
        print("episode end!")
        
        #  compute advantage and return
        success_rate = check_success_rate(obs, args.map_length, args.map_width)
        last_value = agent.get_values(torch.from_numpy(state_engineering(obs, args.map_length, args.map_width, args.num_ped, args.obs_dim)), torch.from_numpy(state_engineering(n_obs, args.map_length, args.map_width, args.num_ped, args.obs_dim)))
