...
from models.model import Encoder, PolicyNetwork_cond, ValueNetwork, lcfNetwork, ValueNetwork_cond
...
class CCPO():
    ...
    def __init__(self, args):
        ...
        # rollout buffer
        self.rollout_buffer = RolloutBuffer(self.buffer_size, self.obs_dim, self.action_dim, device=args.device, gae_lambda=self.gae_lambda, gamma=self.gamma, n_envs=self.n_envs)
        
        # networks
        self.policy_network = PolicyNetwork_cond(args)
        self.conditioned_value = args.conditioned_value
        if args.conditioned_value:
            self.i_value_network = ValueNetwork_cond(args)
            self.n_value_network = ValueNetwork_cond(args)
            self.g_value_network = ValueNetwork_cond(args)
        else:
        ...
        entire_parameters = list(self.policy_network.parameters()) + list(self.i_value_network.parameters())\
                            + list(self.n_value_network.parameters()) + list(self.g_value_network.parameters())
        self.optimizer = torch.optim.Adam(entire_parameters, lr=self.learning_rate)
        

    def evaluate_actions(self, obs, actions, lcf):
        
        obs = obs.to(self.device)
        lcf = lcf.reshape(-1,1)
        
        if self.conditioned_value:
            i_value = self.i_value_network(obs, lcf)
            n_value = self.n_value_network(obs, lcf)
            g_value = self.g_value_network(obs, lcf)
        else:
    ...
    def act(self, obs, lcf):
        ...
        if self.conditioned_value:
            value = self.i_value_network(obs, lcf)
            n_value = self.n_value_network(obs, lcf)
            g_value = self.g_value_network(obs, lcf)
        else:
